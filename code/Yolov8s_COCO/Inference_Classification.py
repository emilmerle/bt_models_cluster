...
# Inference
filename = f"./../../output_data/Inference_Yolov8s-cls_COCO/{device_name}.csv"
os.makedirs(os.path.dirname(filename), exist_ok=True)

# Load a model
if device_name in ["RP4", "RP3"]:
    pre_model = YOLO(
        "./../../models/yolov8s-cls.pt"
    )  # load a pretrained model (recommended for training)
    pre_model.export(format="ncnn")
    model = YOLO("./../../models/yolov8s-cls_ncnn_model")
elif device_name in ["JETSONNANO"]:
    pre_model = YOLO("./../../models/yolov8s-cls.pt")
    pre_model.export(format="engine")  # creates 'yolov8n.engine'
    model = YOLO("./../../models/yolov8s-cls.engine")
else:
